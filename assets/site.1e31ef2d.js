...
function initFacetWizard(lang) {
  ...
  if (!ui.form.dataset.wizardListenersBound) {
    ...
    /* Re-selecting the answer already chosen (after going back with "Previous")
       fires no "change" event: advance on the click as well in that case. */
    ui.form.addEventListener('pointerdown', function(event) {
      var radio = null;
      if (event.target && event.target.matches('input[type="radio"]')) radio = event.target;
      else if (event.target && event.target.closest('label')) {
        var lab = event.target.closest('label');
        radio = (lab.control && lab.control.type === 'radio') ? lab.control : lab.querySelector('input[type="radio"]');
      }
      if (radio) ui.form.dataset.pressedChecked = radio.checked ? radio.value + '|' + radio.name : '';
    });
    ui.form.addEventListener('keydown', function(event) {
      if (event.key === 'Enter' && event.target && event.target.matches('input[type="radio"]') && event.target.checked) {
        event.preventDefault();
        facetWizardAnswered(lang);
      }
    });
    ui.form.addEventListener('click', function(event) {
      ...
      var wasChecked = ui.form.dataset.pressedChecked === radio.value + '|' + radio.name;
      ui.form.dataset.pressedChecked = '';
      if (wasChecked && radio.checked) facetWizardAnswered(lang);
    });
...
function facetWizardAnswered(lang) {
  ...
  var answered = !!step.querySelector('input[type="radio"]:checked');
  if (answered) {
    if (ui.form.dataset.advancePending === 'true') return;
    ui.form.dataset.advancePending = 'true';
    setTimeout(function() {
      ui.form.dataset.advancePending = 'false';
      facetWizardNext(lang);
    }, 800);
  }
}
